...
[#3 那篇](2026-06-22-phpbb-restore-part-03-data-recovery)講完了資料復原。資料層接好之後，下一步是把這些資料放在哪裡跑。

要塞進這台 VPS 的東西有六個 service。phpBB eq2 站一份（PHP 8.1 + Apache）、phpBB eq 站一份（同上）、WordPress 一份（PHP 8.2 + Apache）、phpBB 兩站共用一個 MySQL instance（起頭 8.0、後來升到 8.4）、WordPress 自己一個獨立 MySQL 8.0 instance、最外面一層 Traefik v3 做 reverse proxy 與 HTTPS 終止。六個 container 加起來要在 1 GB RAM 的機器上跑得動，而且還要留 buffer 給 Docker daemon、Linux kernel、定期跑的 cron job。
...
phpBB eq2 跟 phpBB eq 兩個 service 用同一份 Dockerfile 起來，base image 是 `php:8.1-apache`，加了 mysqli、GD、opcache、zip 這幾個 phpBB 跑得起來必要的 PHP extension。兩個 container 共用一個 MySQL instance（phpbb-db，現在跑 8.4），兩站 DB 用不同 schema name 隔離（`eq2_db` 與 `eq_db`），不同 prefix（`phpbb3_` vs `phpbb_`）也降低跨站誤操作的風險。

WordPress 那邊我刻意做了獨立的 db-wordpress instance，跟 phpBB 的 MySQL 完全隔離。為什麼？因為 [#3](2026-06-22-phpbb-restore-part-03-data-recovery) 提過 WordPress 走的是 Y 方案，是個 archive，14 篇 publish post 加 12 user 的小資料量，但它 schema 升級過程動到了 utf8 → utf8mb4 conversion、跑了 4 段 image 切換、總共改過幾百個 schema 變更。這條鏈如果 share MySQL instance 跟 phpBB 的話，任何一個 WordPress 升級腳本意外副作用都可能波及 phpBB。隔離兩個 instance 是用一點記憶體換掉一個 SPOF 風險，划得來。
...
兩個 MySQL instance 起頭都是 8.0，我在 Bluehost 退場前把 phpBB 那個升到了 8.4 LTS。WordPress 那個目前還留在 8.0，等 archive 站長期穩定後再另開一個 maintenance window 處理，沒必要兩個一起冒險。

8.0 升 8.4 官方支援 in-place upgrade，意思是不用 dump-and-restore，直接換 image 啟動 container 就行，新版的 mysqld 會偵測舊資料目錄、跑 schema migration、然後正常啟動。實際步驟是先 docker compose stop 掉兩個 phpBB container、把 db image tag 從 `mysql:8.0` 改成 `mysql:8.4`、docker compose up -d db，in-place upgrade 自動跑（InnoDB redo log、data dictionary 從 80023 升到 80300、server 從 80045 升到 80409），整個過程大概 20 秒，再把 phpBB container 起回來。

升級前一定要先 dump 全套資料庫做 fallback。我用 `mysqldump --all-databases --single-transaction` 跑一份 baseline 存到 `/opt/phpbb/backups/` 底下（約 20 MB）。VPS 本機這份 dump 已經足夠 rollback，所以沒有另外去 DigitalOcean dashboard 取 droplet snapshot。如果 in-place migration 出意外，從這份 dump restore 回 8.0 就行。

升上 8.4 之後驗了五個 gate 才算數：VERSION 確認跑的是 8.4.9、GRANT 權限正常、兩站資料完整性（post 數與 table 數升前後一致）、HTTPS 200、以及 ACP 裡 Calendar extension 的 disable 再 enable 能跑過 migrator，最後這條同時驗證了 schema-scoped 的 `GRANT ALL` 真的涵蓋 DDL 權限。五個都綠才收工。GRANT 這件事在 8.4 升級之前就單獨處理過一輪，牽出一個「phpBB 到底該用多大權限的 DB 帳號」的問題，下一節單獨講。

這個升級因為涉及 production 切換、有不可逆風險，我排在 Bluehost 訂閱失效前的最後一週做，本機先 dry-run 預演過一次，正式上線一次就過、沒有用到 rollback。後來又補了一輪收斂，把 phpBB 連線的 auth plugin 從舊的 `mysql_native_password` 換成 8.4 預設的 `caching_sha2_password`，順手確認了 phpBB 的 PHP 8.1 mysqlnd 對新 plugin 走 TCP 連線一切正常。
